...
import { colors, mediaSizes, spacing } from "../data/styleVariables.js";
...
const CardUserIcons = ({ movieId, posterPath }: CardUserIcons) => {
  ...
  const [movieWished, setMovieWished] = useState(
    currentMovie?.isWished || false
  );
  const [movieLiked, setMovieLiked] = useState(currentMovie?.isLiked || false);
  const [movieDisliked, setMovieDisliked] = useState(
  ...
  return (
    <>
      <IconsContainer>
        {movieWished ? (
          <Icon
            onClick={toggleWishedList}
            className="on fa-solid fa-eye"
            title="Je veux voir !"
          />
        ) : (
          <Icon
            onClick={toggleWishedList}
            className="off fa-regular fa-eye"
            title="Je veux voir !"
          />
        )}
        ...
        {movieLiked ? (
          ...
          <Icon
            onClick={toggleLikedList}
            className="off fa-regular fa-thumbs-up"
            title="J'ai aimé !"
          />
        )}
        ...
        {movieDisliked ? (
          ...
          <Icon
            onClick={toggleDislikedList}
            className="off fa-regular fa-thumbs-down"
            title="Je n'ai pas aimé !"
          />
        )}
      </IconsContainer>
